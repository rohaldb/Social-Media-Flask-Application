...
for z_id in sorted(os.listdir(students_dir)):
	...
	while pathlib.Path(os.path.join(students_dir, z_id, "%d.txt" % post_counter)).is_file():
		...
		c.execute("INSERT INTO posts (id, user, created_at, message, latitude, longitude, type, path) VALUES (?, ?, DATETIME(?), ?, ?, ?, ?, ?)", (post_id, z_id, created_at[:-5], message, latitude, longitude, "text", os.path.join(students_dir, z_id, "%d.txt" % post_counter)))
		...
		while pathlib.Path(os.path.join(students_dir, z_id, "%d-%d.txt" % (post_counter, comment_counter))).is_file():
			...
			c.execute("INSERT INTO comments (id, post, user, created_at, message, type, path) VALUES (?, ?, ?, DATETIME(?), ?, ?, ?)", (comment_id, post_id, commenter_z_id, created_at[:-5], message, "text", os.path.join(students_dir, z_id, "%d-%d.txt" % (post_counter, comment_counter))))
			...
			while pathlib.Path(os.path.join(students_dir, z_id, "%d-%d-%d.txt" % (post_counter, comment_counter, reply_counter))).is_file():
				...
				c.execute("INSERT INTO replies (id, comment, user, created_at, post, message, type,  path) VALUES (?, ?, ?, DATETIME(?), ?,?,  ?, ?)", (reply_id, comment_id, replier_z_id, created_at[:-5], post_id, message, "text", os.path.join(students_dir, z_id, "%d-%d-%d.txt" % (post_counter, comment_counter, reply_counter))))
				reply_counter+=1
			comment_counter+=1
		post_counter+=1
